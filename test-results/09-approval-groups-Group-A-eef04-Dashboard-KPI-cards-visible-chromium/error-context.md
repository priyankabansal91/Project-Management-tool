...
- Name: 09-approval-groups.spec.ts >> Group Approval Dashboard >> KPI cards visible
- Location: e2e\09-approval-groups.spec.ts:172:7
...
Locator: getByText('In Progress')
Expected: visible
Error: strict mode violation: getByText('In Progress') resolved to 2 elements:
    1) <div class="text-sm text-gray-500">In Progress</div> aka getByText('In Progress', { exact: true })
    2) <button class="px-3 py-1.5 rounded-md text-sm font-medium transition-all text-gray-500 hover:text-gray-700">…</button> aka getByRole('button', { name: 'In Progress (2)' })

Call log:
  - Expect "toBeVisible" with timeout 10000ms
  - waiting for getByText('In Progress')
