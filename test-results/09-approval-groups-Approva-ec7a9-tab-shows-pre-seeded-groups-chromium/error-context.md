...
- Name: 09-approval-groups.spec.ts >> Approval Group Configuration >> Groups tab shows pre-seeded groups
- Location: e2e\09-approval-groups.spec.ts:21:7
...
Locator: getByText('Division Head')
Expected: visible
Error: strict mode violation: getByText('Division Head') resolved to 2 elements:
    1) <div class="text-xs text-gray-500">…</div> aka getByText('Division Head · division_admin')
    2) <span class="font-semibold text-gray-900">Division Head</span> aka getByText('Division Head', { exact: true })
