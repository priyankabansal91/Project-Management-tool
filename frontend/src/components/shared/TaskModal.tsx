...
import { X, Bold, Italic, List, Link2, Eye, Code, ChevronDown, Paperclip, FileText, FileSpreadsheet, Image, File, Upload, Flag, AlertCircle, MonitorCheck } from 'lucide-react';
import type { Task, WorkflowStatus } from '@/types';
import { IT_MODE, IT_ENVIRONMENTS, IT_CHANGE_TYPES } from '@/config/itMode';
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[5vh] bg-black/50" onClick={onClose} onKeyDown={handleKeyDown}>
      <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-xl bg-card shadow-2xl border" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* IT-Specific Fields — visible only in IT deployment */}
          {IT_MODE && (
            <div className="px-6 pb-4">
              <div className="border rounded-lg overflow-hidden">
                <div className="flex items-center gap-2 bg-blue-50 dark:bg-blue-950/30 px-4 py-2.5 border-b">
                  <MonitorCheck className="h-3.5 w-3.5 text-blue-600" />
                  <span className="text-xs font-semibold text-blue-700 dark:text-blue-400 uppercase tracking-wider">IT Details</span>
                </div>
                <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <select
                      value={(form.custom_fields?.it_environment as string) || ''}
                      onChange={(e) => updateField('custom_fields', { ...form.custom_fields, it_environment: e.target.value })}
                      className="w-full rounded-md border bg-background px-3 py-2 text-sm appearance-none"
                    >
                      <option value="">Select environment...</option>
                      {IT_ENVIRONMENTS.map((env) => <option key={env} value={env}>{env}</option>)}
                    </select>
                  </div>
                  {/* Change Type */}
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Change Type</label>
                    <select
                      value={(form.custom_fields?.it_change_type as string) || ''}
                      onChange={(e) => updateField('custom_fields', { ...form.custom_fields, it_change_type: e.target.value })}
                      className="w-full rounded-md border bg-background px-3 py-2 text-sm appearance-none"
                    >
                      <option value="">Select type...</option>
                      {IT_CHANGE_TYPES.map((ct) => <option key={ct} value={ct}>{ct}</option>)}
                    </select>
                  </div>
                  {/* Ticket Reference */}
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Ticket Reference</label>
                    <Input
                      placeholder="e.g. JIRA-1234, SNOW-5678"
                      value={(form.custom_fields?.it_ticket_ref as string) || ''}
                      onChange={(e) => updateField('custom_fields', { ...form.custom_fields, it_ticket_ref: e.target.value })}
                      className="text-sm"
                    />
                  </div>
                  {/* Release Tag */}
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Release Tag</label>
                    <Input
                      placeholder="e.g. v2.3.1, Release-Oct-2026"
                      value={(form.custom_fields?.it_release_tag as string) || ''}
                      onChange={(e) => updateField('custom_fields', { ...form.custom_fields, it_release_tag: e.target.value })}
                      className="text-sm"
                    />
                  </div>
                </div>
